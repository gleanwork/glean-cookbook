...
Same Globex account page UX as Path A, but journey summary, saved prompts, and
...
1. Create an **Account Brief** agent in Agent Builder with template sections
   (Overview, Renewal, Risks, Security) and retrieval on Acme sales docs.
...
`input: { account: "Globex", question }` instead.
