...
`server.ts` is a small Node HTTP server. `GET /` serves the page. `POST /api/ask` calls the Chat API through `glean.chat.create` with `stream:false` and `store:false`. It then extracts:
