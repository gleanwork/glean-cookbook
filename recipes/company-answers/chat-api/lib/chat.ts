...
export async function askPlatformChat(
  ...
  try {
    response = await glean.chat.create(buildPlatformChatRequest(question));
  } catch (error) {
  ...
  if (typeof response === 'string') {
    throw new Error('Chat API returned a stream for a non-stream request.');
  }
  const parsed = parsePlatformChatResponse(response);
  if (!parsed.answer.trim()) {
    if (attempt < 2) return askPlatformChat(question, attempt + 1);
    throw new Error(
