...
describe('resolveServerURL', () => {
  ...
  test('uses email discovery instead of a saved tenant', async () => {
    const discover = vi.fn().mockResolvedValue({
      instance: 'new-tenant',
      backend: 'https://new-tenant-be.glean.com',
    });
      ...
      resolveServerURL(
        {
          ...
          savedServerURL: 'https://old-tenant-be.glean.com',
        },
        discover,
      ),
    ).resolves.toBe('https://new-tenant-be.glean.com');
    expect(discover).toHaveBeenCalledWith('person@example.com');
  });
  ...
  test('falls back to a saved backend when no explicit input is provided', async () => {
    const discover = vi.fn();

    await expect(
      resolveServerURL(
        { savedServerURL: 'https://saved-be.glean.com' },
        discover,
      ),
    ).resolves.toBe('https://saved-be.glean.com');
    expect(discover).not.toHaveBeenCalled();
  });
