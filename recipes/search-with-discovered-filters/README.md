...
The recipe uses `@gleanwork/auth` for tenant discovery, OAuth login, secure credential storage, and automatic token refresh, then `@gleanwork/api-client` 0.20.14 for typed Search Filters and Platform Search. Each request attempt has a 30-second timeout, with bounded exponential backoff for transient API and connection failures.
