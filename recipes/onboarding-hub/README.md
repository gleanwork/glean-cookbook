...
- **[`web-sdk/`](web-sdk/)** — Glean owns the chat UI via `renderChat` (SSO cookie auth). Steps from `public/steps.json`.
- **[`platform-chat/`](platform-chat/)** — you own the UI; the server calls Platform Chat (`POST /api/chat`, experimental). Steps from `GLEAN_ONBOARDING_STEPS_*`.
