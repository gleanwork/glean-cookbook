...
test('fixture walkthrough and optional live auth keep separate contracts', () => {
  ...
  assert.equal(pkg.scripts?.login, 'glean-auth login --scopes SEARCH,CHAT');
  assert.equal(
    pkg.scripts?.['login:agent'],
    'glean-auth login --scopes SEARCH,CHAT,AGENTS',
  );
  assert.match(recipe, /"requiredScopes": \[\]/u);
  assert.match(recipe, /"authMethod": \["none"\]/u);
  assert.equal(recipe.match(/cd oncall-copilot &&/gu)?.length, 1);
