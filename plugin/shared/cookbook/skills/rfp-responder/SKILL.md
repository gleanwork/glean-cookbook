...
- For a live run: company content indexed in Glean, and comma-separated Glean URL prefixes your security team has cleared for customer-facing answers. You put those prefixes in RFP_APPROVED_SOURCE_PREFIXES. Signing in does not fill them in
...
- Which Glean URL prefixes may be quoted to a customer? Use comma-separated prefixes your security team has cleared for external answers.
...
2. **Install dependencies**
   Installs the small local server that drafts and reviews questionnaire answers.
...
3. **See it work before you connect anything**
   Runs the whole flow against recorded Chat responses, so you can see what it produces before you connect anything. This needs no Glean credentials.
