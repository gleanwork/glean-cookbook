...
- **IT helpdesk deflection page, no code, on Lovable** (`/cookbook:no-code-it-helpdesk-lovable`) — Prompt Lovable into a private, single-user IT helpdesk prototype on the Glean Chat API, with zero hand-written backend.
- **PTO and benefits lookup, no code, on Replit** (`/cookbook:no-code-pto-lookup-replit`) — Prompt Replit Agent into a private, single-user HR lookup prototype on the Glean Chat API, with zero hand-written backend.
