...
test('supports a standalone cd step', (t) => {
  const result = checkCommands(t, [scaffold, 'cd demo', 'npm test']);
  assert.equal(result.status, 0, result.stderr);
});

test('does not skip unsafe commands in a scaffold block or persist a subshell', (t) => {
  for (const command of ['npm install', '(cd demo && npm install)']) {
    const result = checkCommands(t, [`${scaffold}\n${command}`, 'npm test']);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /must enter demo/);
  }
});

test('raw approval-recipe shell commands stay inside the scaffold directory', (t) => {
  const approvalRecipe = JSON.parse(
    fs.readFileSync(
      path.join(repoRoot, 'recipes/human-in-the-loop-agent/recipe.json'),
      'utf8',
    ),
  );
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), 'approval-step-sequence-'),
  );
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const commands = approvalRecipe.steps.flatMap((step) =>
  ...
  const expected = path.join(fs.realpathSync(root), 'human-in-the-loop-agent');
  assert.deepEqual(output.trim().split('\n'), Array(6).fill(expected));
  assert.equal(
    approvalRecipe.execution.auth[0].setupCommand,
    approvalRecipe.steps.find((step) => step.kind === 'authenticate').command,
  );
  assert.equal(
    approvalRecipe.execution.run.command,
    approvalRecipe.steps.find((step) => step.kind === 'run').command,
  );
});
